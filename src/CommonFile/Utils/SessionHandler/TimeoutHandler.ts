      ...
      async (TurnContext: any) => {
        await new Promise((resolve) => {
          const handler = setTimeout(async (resolve) => {
            ...
            dialogSet.add(
              DialogFactory.getRegistry().get("bot.session.handler")
            );

            const dialogContext = await dialogSet.createContext(TurnContext);

            TimerCache.getInstance().cache.set(
              TurnContext.activity.conversation.id,
              { isPromptTimerActive: true }
            );

            dialogContext.replaceDialog("bot.session.handler");
            dialogContext.cancelAllDialogs(true);
          }, 10000);
          this.TimeoutStorage.setPromptHandlerFor(
    ...
    context.adapter.continueConversation(
      ...
      async (TurnContext: any) => {
        await new Promise((resolve) => {
          ...
          this.TimeoutStorage.setEndConvoHandlerFor(
            context.activity.conversation.id,
            handler
          );
        });
